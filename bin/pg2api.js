  ...
  if (!err && res.rows.length > 0) {
    client.query('select grantee, table_name, privilege_type from information_schema.role_table_grants\n' +
      ...
      if (err) {
        ...
        const paths = {}
        const ddls = res.rows.reduce(function (rv, x) {
        ...
        Object.keys(ddls).forEach(function (tn) {
          ...
          const is_protected = isProtected(tn, auth.rows, auth_role)
          const role = is_protected ? auth_role : pub_role
          paths[path] = {
            name: toName(tn),
            path: path,
        ...
        } else { console.log(JSON.stringify(schema)) }
      }
      client.end()
    })
  } else {
