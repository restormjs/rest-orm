  ...
  scenarios.tests.forEach((t) => {
    it(`Test ${t.args.m} ${t.args.url} responds ${t.response.status}`, function (done) {
      ...
      orm.execute = function (query, done) {
        ...
        if (t.orm.filters) { expect(query.filters).to.deep.equal(t.orm.filters) } else { expect(query.filters).to.be.empty }
        if (t.orm.payload) { expect(query.payload).to.deep.equal(t.orm.payload) } else { expect(query.payload).to.be.undefined }
